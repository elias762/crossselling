...
function getProductRules(db, whereClause = '', params = []) {
    const rules = query(db, `SELECT id, trigger_service, reason, active FROM product_rules ${whereClause} ORDER BY trigger_service`, params);
...
module.exports = async (req, res) => {
    try {
        ...
        if (req.method === 'GET') {
            ...
            const ruleId = run(db, 'INSERT INTO product_rules (trigger_service, reason, active) VALUES (?, ?, ?)',
                [trigger, reason || null, active !== false ? 1 : 0]);

            for (const suggestion of suggestions) {
                run(db, 'INSERT INTO product_rule_suggestions (rule_id, suggested_product) VALUES (?, ?)', [ruleId, suggestion]);
            }

            const rules = getProductRules(db, 'WHERE id = ?', [ruleId]);
